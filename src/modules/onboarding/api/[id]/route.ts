...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await db.onboarding.findUnique({ where: { id }, include: { employee: true } })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await db.onboarding.findUnique({ where: { id } })
    if (!existing) return NextResponse.json({ error: 'یافت نشد' }, { status: 404 })
    const item = await db.onboarding.update({ where: { id }, data: body })
    return NextResponse.json(item)
  } catch (error) { return NextResponse.json({ error: 'خطا' }, { status: 500 }) }
}

...
import { CreditCard, Calculator, Search, X, ChevronDown, Calendar } from 'lucide-react'
...
export function LoanFormDialog({
  ...
  // محاسبه مبلغ هر قسط
  const monthlyInstallment = loanForm.amount > 0 && loanForm.installments > 0
    ? Math.round(loanForm.amount / loanForm.installments)
    : 0
                    ...
                    filteredEmployees.map(emp => (
                      <div
                        ...
                        className={`flex items-center gap-3 px-2 py-2 rounded-md cursor-pointer transition-colors ${
                          loanForm.employeeId === emp.id
                            ? 'bg-sky-50 dark:bg-sky-950/30'
                            : 'hover:bg-muted/50'
                        }`}
                        onClick={() => {
                          onFormChange({ ...loanForm, employeeId: emp.id })
                          setEmployeeSearch('')
    ...
    : 'bg-gradient-to-br from-emerald-400 to-teal-500'
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                type="text"
                inputMode="numeric"
                value={loanForm.amount ? toPersianNumber(String(loanForm.amount)) : ''}
                onChange={e => {
                  const englishNumber = toEnglishNumber(e.target.value)
                  const numeric = englishNumber.replace(/[^0-9]/g, '')
                  onFormChange({ ...loanForm, amount: numeric ? +numeric : 0 })
                }}
                className="text-xs"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                type="text"
                inputMode="numeric"
                value={loanForm.installments ? toPersianNumber(String(loanForm.installments)) : ''}
                onChange={e => {
                  const englishNumber = toEnglishNumber(e.target.value)
                  const numeric = englishNumber.replace(/[^0-9]/g, '')
                  onFormChange({ ...loanForm, installments: numeric ? +numeric : 0 })
                }}
                className="text-xs"
                placeholder={toPersianNumber('۱۲')}
              />
            </div>
            <div>
        ...
        <DialogFooter className="gap-2">
          ...
          <Button size="sm" onClick={onSave} disabled={saving} className="text-xs">
            {saving ? toPersianNumber('ذخیره...') : editingLoan ? toPersianNumber('بروزرسانی') : toPersianNumber('ذخیره')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
